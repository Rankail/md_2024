...
Node* InputReader::readNodeLine(const std::string& line, unsigned lineNum) {
    ...
    if (data.size() != 4) {
        TET_ERROR("Incorrect node-input-data at line {}. Got {} data-points instead of 4.", lineNum, data.size());
        return nullptr;
    }
...
uPair InputReader::readEdgeLine(const std::string& line,
    ...
    const auto& data = splitAt(line, " \t");
    if (data.size() != 2) {
        TET_ERROR("Incorrect edge-input-data. Got {} data-points instead of 2.", data.size());
        return {-1, -1};
    }
