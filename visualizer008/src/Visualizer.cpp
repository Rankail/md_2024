...
bool Visualizer::init(const Vec2u& size) {
    ...
    window = new Window("MD 2024", {-1, -1}, size, WindowFlag::SHOWN | WindowFlag::RESIZABLE);
        ...
        switch(event.type) {
            ...
            case SDL_WINDOWEVENT:
                if (event.window.type == SDL_WINDOWEVENT_RESIZED) {
                    TET_INFO("Resized");
                    Vec2u size{event.window.data1, event.window.data2};
                    solver.setTargetSize(size);
                }
...
//        SDL_GetKeyName(event.key.keysym.sym));
    switch(event.key.keysym.scancode) {
        case SDL_SCANCODE_A: solver.run(1); break;
        case SDL_SCANCODE_S: solver.run(10); break;
        case SDL_SCANCODE_D: solver.run(100); break;
        case SDL_SCANCODE_F: solver.run(1000); break;
        case SDL_SCANCODE_G: solver.run(10000); break;

        case SDL_SCANCODE_UP: solver.moveOffset({0., -10.}); break;
        case SDL_SCANCODE_DOWN: solver.moveOffset({0., 10.}); break;
        case SDL_SCANCODE_LEFT: solver.moveOffset({-10., 0.}); break;
        case SDL_SCANCODE_RIGHT: solver.moveOffset({10., 0.}); break;
        case SDL_SCANCODE_RIGHTBRACKET: solver.decreaseZoom(); break;
        case SDL_SCANCODE_BACKSLASH: solver.increaseZoom(); break;
        default: return;
    }
    graphicData = solver.getGraphicData();
}
...
void Visualizer::onKeyUp(const SDL_Event &event) {
    switch (event.key.keysym.scancode) {
        ...
        case SDL_SCANCODE_Q: solver.run(1); break;
        case SDL_SCANCODE_W: solver.run(10); break;
        case SDL_SCANCODE_E: solver.run(100); break;
        case SDL_SCANCODE_R: solver.run(1000); break;
        case SDL_SCANCODE_T: solver.run(10000); break;
        ...
        case SDL_SCANCODE_I: solver.increaseStepSize(); break;
        case SDL_SCANCODE_K: solver.decreaseStepSize(); break;
